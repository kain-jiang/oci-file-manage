...
`oci-file-manage` 是 [tiramission/oci-sync](https://github.com/tiramission/oci-sync)(Go CLI)的 Android 移植版。核心目标:
...
**导航模式**(以 `ui-design/` 页面原型为准):单 Activity + Navigation Compose,底部三 Tab
「首页/仓库/历史」**贯穿全部页面**(含 push/pull/设置/快捷仓库等子页,子页同时保留返回键)。
