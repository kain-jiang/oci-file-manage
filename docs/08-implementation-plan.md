...
> UI 以 `ui-design/`(Material 3 页面原型)为准,与功能文档配套。

任务:
- [ ] `MainActivity` + NavHost + 底部导航(首页/仓库/历史,三 Tab **贯穿全部页面**,含子页)
- [ ] HomeScreen(快捷仓库卡片 + 快速操作:推送文件/拉取文件;无设置齿轮,入口方式以设计稿为准)
- [ ] SettingsScreen(凭据管理 + 快捷仓库管理 + 关于)
- [ ] 空状态、深色模式、i18n 字符串(中/英)

验收:
- [ ] 应用可安装,三 Tab 可切换(含子页底部导航)
- [ ] 添加/删除/**编辑** shortcut 与凭据落盘生效
...
- [ ] PushScreen(本地路径选择 + 仓库引用 + 加密口令[显示/隐藏] + 标签列表 + 进度/取消)
- [ ] PullScreen(仓库引用 + 目标目录 + 解密口令[加密检测后提示] + 进度)
- [ ] ListScreen(查询 + **标签筛选 chips**[全部/预设标签] + 表格 + 行操作:拉取/删除/标签弹窗)
- [ ] ShortcutDetailScreen(tag 列表 + **已加密标记** + 行操作 + **编辑 shortcut** + 推送新版本)
- [ ] HistoryScreen(列表 + 类型筛选 chips + 清空)
- [ ] 大文件前台服务 + 通知进度(≥20MB)
